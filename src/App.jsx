...
function App() {
  ...
  const filteredArr = product.filter((data) => {
    const matchesSearch = data.title.toLowerCase().includes(search.toLowerCase());
    const matchesCategory = data.category.toLowerCase().includes(category.toLowerCase());
    const matchesPrice = price === "" || data.price <= parseFloat(price);
